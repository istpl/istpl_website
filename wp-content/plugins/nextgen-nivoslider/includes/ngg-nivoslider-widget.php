...
Class NGG_NivoSlider_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$s = array(
				...
				'showGalleryTitle' => $this->get_val( $instance, 'showgallerytitle' ),
				'showGalleryDesc' => $this->get_val( $instance, 'showgallerydesc' ),
		);
		...
		if ( $pSize > 0 ) {
			...
			// Add theme class and title/desc if applicable
			...
			$addInfo = false;
			if ( $s['showGalleryDesc'] || $s['showGalleryTitle'] ) {
				$addInfo = true;
				$output .= '#REPLACETHIS#';
			}
			...
			foreach ( $results as $result ) {
				...
				//
				if ( $addInfo ) {
					$replaceString = '<div class="nivoSlider-galleryInfo">';
					if ( $s['showGalleryTitle'] ) {
						$replaceString .= '<h3>' . trim( $gallery->title ) . '</h3>';
					}
					if ( $s['showGalleryDesc'] ) {
						$replaceString .= '<p>' . trim( $gallery->galdesc ) . '</p>';
					}
					$replaceString .= '</div>';
					$output = str_replace( '#REPLACETHIS#', $replaceString, $output );
				}
		...
		if ( $pSize <= 1 ) {
			$ns['startSlide'] = '0';
			$ns['directionNav'] = 'false';
			$ns['controlNav'] = 'false';
			$ns['keyboardNav'] = 'false';
			$ns['pauseOnHover'] = 'false';
			$ns['manualAdvance'] = 'false';
			$ns['beforeChange'] = '';
			$ns['afterChange'] = '';
			$ns['slideshowEnd'] = '';
			$ns['lastSlide'] = '';
